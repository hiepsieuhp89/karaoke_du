        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.status){
                    ...
                    $.admin.toastr.error('Lá»—i khĂ´ng xĂ¡c Ä‘á»‹nh, hĂ£y thá»­ láº¡i', '', {positionClass:"toast-top-right"});;
            }
        });
            ...
            success: function(res) {
                ...
                $.map($('tbody tr'), function(n) {
                    if (jQuery.inArray(($(n).find('.column-deviceCode a')).attr('data-content'), deviceCodes) == -1) {
                        ...
                        $(n).find('.column-status').html('<b class="text-danger">KhĂ´ng hoáº¡t Ä‘á»™ng</b>');
                    }
                });
            }
        });
